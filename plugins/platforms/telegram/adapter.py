...
class TelegramAdapter(BasePlatformAdapter):
    ...
    async def _send_chunk_with_retries(
        ...
        for _send_attempt in range(3):
            try:
                ...
                if index == 0 and (metadata or {}).get("_review_reply_markup") is not None:
                    send_kwargs["reply_markup"] = metadata["_review_reply_markup"]
    ...
    async def _send_text_locked(
        ...
        try:
            ...
            review_id = None if (metadata or {}).get("postgen_candidate") else self._review_candidate_id(metadata)
            review_row = await asyncio.to_thread(self._resolve_review_candidate, review_id) if review_id else None
            review_markup = self._review_candidate_reply_markup(review_row)
            if review_markup is not None:
                metadata = {**(metadata or {}), "_review_reply_markup": review_markup}
            if review_markup is None and self._should_attempt_rich(content, metadata=metadata):
            ...
            self._log_review_candidate_delivery(review_row, attached=review_markup is not None and bool(delivered), message_id=delivered[0] if delivered else None, chat_id=chat_id, media_kind="text")
            return result
        except Exception as e:
    ...
    async def _handle_postgen_callback(self, query, data: str, cb: Dict[str, Any]) -> None:
        ...
        parsed = _parse_postgen_candidate_callback(data)
        if parsed is None:
            await query.answer(text="Invalid candidate action.")
            return
        action, candidate_id, slide_index = parsed
    ...
    async def _send_media(
        ...
        review_id = None if (metadata or {}).get("postgen_candidate") else self._review_candidate_id(metadata)
        row = await asyncio.to_thread(self._resolve_review_candidate, review_id) if review_id else None
        markup = self._review_candidate_reply_markup(row)
        ...
        try:
            ...
            self._log_review_candidate_delivery(row, attached=False, chat_id=chat_id, media_kind=media_label)
            raise
        self._log_review_candidate_delivery(row, attached=markup is not None, message_id=msg.message_id, chat_id=chat_id, media_kind=media_label)
        return msg
    ...
    async def send_multiple_images(
        ...
        if candidate_row is not None and len(photos) > 1:
            ...
            if (
                ...
                return SendResult(success=False, error="Postgen carousel media count mismatch")

        from urllib.parse import unquote as _unquote
        if any((metadata or {}).get(key) for key in ("postgen_candidate", "review_candidate", "review_candidate_id")) and len(photos) == 1:
        ...
        for chunk_idx, chunk in enumerate(chunks):
            ...
            try:
                ...
                fallback = await super().send_multiple_images(chat_id, chunk, fallback_metadata, human_delay=human_delay)
        ...
        if candidate_row is not None:
            ...
            if reply_markup is None:
                ...
                return SendResult(success=False, error="Invalid Postgen carousel keyboard")
